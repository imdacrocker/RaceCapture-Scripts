...
function onTick()
    ...
    if (moving == false) then -- If the car was not moving last time we checked, check to see if it's moving
        if ((getGpsSpeed() > 10) and (rpm > 500)) then -- If the car has begun to move while running, then do these things
            moving = true -- Set Moving to true
            sendCommand(string.char(0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x00, 0x53, 0x48, 0x02),
                '1') -- Start GoPro
            ...
            if ((getUptime() - stopTime) > 120000) then -- If the car has been stopped for 2 minutes
                ...
                maxRPM = rpm -- Reset the maxRpm channel
            end
        end
    end
    if (moving) then -- If the car was moving last time we checked, check to see if it's stopped
        ...
        if (getGpsSpeed() == 0 and rpm == 0) then -- Use getGpsSpeed() AND getChannel("RPM") for this
            moving = false -- Set the car to not moving
            if ((getUptime() - stopTime) > 120000) then
                sendCommand(string.char(0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x00, 0x53, 0x48,
                    0x00), '1') -- Stop GoPro
                stopLogging() -- Stop logging if the car has been stopped for 2 minutes or more
            end
        end
    end
    -- checkRunning()
    ...
    if (running) then
        engineOffTime = getUptime()
        if rpm == 0 then -- If the car is not running
            running = false
        end
    end

    -- maxRev
    if rpm > getChannel("MaxRpm") then
        setChannel(m, rpm)
        if (moving) then
            if (rpm > 6500) then
            end
    ...
    -- checkGear()
    local gearPos = calcGear(58.42, 4.10, 3.83, 2.2, 1.4, 1.0, .89)
    if (gearPos == nil) then
    ...
    -- checkOil()
    local oilPress = getChannel("OilPress")
    
    if (oilPress < 5 and rpm <= 3000) or (rpm > 3000 and oilPress < 35) 
        then
